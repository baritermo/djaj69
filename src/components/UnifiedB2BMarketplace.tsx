...
export default function UnifiedB2BMarketplace({
  ...
  const [offers, setOffers] = useState<any[]>(INITIAL_MARKET_OFFERS);
  ...
  const fetchB2BOffers = useCallback(async () => {
    try {
      ...
      if (data.status === 'success' && Array.isArray(data.offers) && data.offers.length > 0) {
        setOffers(data.offers);
      } else {
        // Fallback filter over initial market offers
        const filtered = INITIAL_MARKET_OFFERS.filter((o) => {
          if (selectedCategory !== 'all' && o.offerCategory !== selectedCategory) return false;
          if (selectedIntent !== 'all' && o.intentType !== selectedIntent) return false;
          if (selectedWilaya !== 'all' && o.wilayaCode !== selectedWilaya && o.wilayaCode !== String(selectedWilaya).padStart(2, '0')) return false;
          if (searchQuery.trim() !== '') {
